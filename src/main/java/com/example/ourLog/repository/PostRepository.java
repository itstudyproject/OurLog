...
public interface PostRepository extends JpaRepository<Post, Long>, SearchRepository {
  ...
  @Query("select po, pi, u, count(distinct r), up, t from Post po " +
          "left outer join Picture pi on pi.post = po " +
...
""")
  ...
  @Query("SELECT p FROM Post p " +
          "JOIN p.user u " + // Post와 User를 조인
          "LEFT JOIN Follow f ON f.toUser = u " + // User와 Follow를 LEFT JOIN
          "WHERE p.boardNo = 5 " +
          "GROUP BY p " + // p (Post 엔티티 전체)를 그룹화
          "ORDER BY COUNT(f) DESC, p.regDate DESC")
